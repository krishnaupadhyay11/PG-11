...
export default function Header() {
  ...
  return (
    <div className='w-full flex flex-col gap-4'>
      <header className="w-full sticky strm top-0 h-[72px] py-2 px-4 md:px-6 flex items-center justify-between z-[9999]">
        ...
        <div className='hidden md:flex items-center justify-end'>
          <nav className='flex items-center'>
            <ul className='flex items-center'>
                {navLinks.map((navLink) => (
                  <li key={navLink.title}>
                    <Link to={navLink.link} className={`${navLink.link === activeNavLink ? 'text-logo2 hover:text-logo2' : 'hover:text-logo1'} text-black text-[18px] p-2 px-4 transition-all duration-200`} onClick={() => window.scroll(0,0)}>{navLink.title}</Link>
                  </li>
                ))}
            </ul>
          </nav>
      ...
      {!home && (
        <hr className='w-full p-2 bg-logo2' />
      )}
    </div>
